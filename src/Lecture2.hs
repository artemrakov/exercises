...
merge = helper []
  where
    ...
    helper acc [] xs = acc ++ xs
    helper acc list1@(x1:xs1) list2@(x2:xs2) | x1 < x2 = helper (acc ++ [x1]) xs1 list2
                                             | otherwise = helper (acc ++ [x2]) list1 xs2
...
merge = helper []
  where
    helper acc xs [] = acc ++ xs
    helper acc [] xs = acc ++ xs
    helper acc (x1:xs1) (x2:xs2) | x1 < x2 = helper (acc ++ [x1]) xs1 (x2:xs2)
                                             | otherwise = helper (acc ++ [x2]) (x1:xs1) xs2
